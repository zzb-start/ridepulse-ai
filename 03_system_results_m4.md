...
| 分类模型 | 主分类模型(OpenAI 兼容 API,37/37) |
| 复判模型 | 第二模型,与主分类模型不同(独立第二轮,37/37) |
...
- 人工复核队列:12 条冲突已全部裁决(2026-08-14,`adjudication_record.csv` 逐条留档),25 条 approved
...
全局最高分簇:Garmin 固件相关问题大簇(77 分,P1,6 条证据)——属竞品洞察,不进入我方 backlog;我方产品最高分簇:设备与 App 同步异常(61 分,P2,4 条证据)——连接与同步类问题为当前最显著需求(业务复核口径保留 P1 优先级,见方案 §5)。

## 4. 评测结果(与 20 条 gold 标注比对)

配对 18/20(`F0030`/`F0031` 在 gold 标注中但未入选 DATASET v1 正式 37 条,已核对标注交付一致,属数据集版本差异):
...
3. **人工复核队列**:`human_final_outputs.csv` 原 12 条 pending 已于 2026-08-14 全部裁决(27 处字段变更,依据见 `adjudication_record.md`;5 条以 gold 为准、7 条按复判+原始证据严重度)。
